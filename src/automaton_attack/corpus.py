...
def custom_vocab_path() -> Path:
    return Path(str(resources.files("automaton_attack.data")
...
def write_vocab() -> Path:
    ...
    header = ("# Generated by `automaton update-data` from "
              "odota/dotaconstants -- do not edit.\n"
              "# Add words to src/automaton_attack/data/custom_vocab.txt "
              "instead.\n")
...
def write_phrases() -> Path:
    ...
    path = phrases_path()
    path.parent.mkdir(parents=True, exist_ok=True)
    header = ("# Generated by `automaton update-data` from mdiller/dotabase "
              "-- do not edit.\n")
    path.write_text(header + "\n".join(phrases) + "\n", encoding="utf-8")
...
def ensure(with_phrases: bool = True, force: bool = False) -> tuple[Path, Path | None]:
    """Return corpus paths, building anything missing.

    First run downloads and builds; after that it is a pair of stat calls.
    """
    need_vocab = force or not vocab_path().exists()
    need_phrases = with_phrases and (force or not phrases_path().exists())
