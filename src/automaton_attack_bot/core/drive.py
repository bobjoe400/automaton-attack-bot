...
def _drive_loop(frames, lexicon, backend, settings, typist, engine, tracker,
    ...
    multiplier_read = None      # (frame timestamp, future) in flight
    ...
    # Multiplier telemetry cadence. The combo transitions in the log are
    # only as fresh as the reads: the tiny x-glyphs fail OCR often, so a
    # slow cadence stamped x3.0 seconds after it actually hit. Reads run
    # on their own worker (never the decision path) and each log line is
    # stamped with the FRAME time that was read, not the discovery time.
    MULTIPLIER_INTERVAL = 0.15 if scan_pipeline is not None else 0.5

    telemetry_pool = None
    if scan_pipeline is not None:
        from concurrent.futures import ThreadPoolExecutor

        telemetry_pool = ThreadPoolExecutor(max_workers=1,
                                            thread_name_prefix="telemetry")

    def read_multiplier_change(timestamp, frame):
        """Yield (frame_time, new_multiplier) when a change is observed."""
        nonlocal last_multiplier_read, multiplier_read
        if telemetry_pool is None:
            if timestamp - last_multiplier_read >= MULTIPLIER_INTERVAL:
                last_multiplier_read = timestamp
                value = tracker.read_multiplier(frame)
                if value is not None and value != last_multiplier:
                    return timestamp, value
            return None
        if multiplier_read is not None and multiplier_read[1].done():
            read_ts, future = multiplier_read
            multiplier_read = None
            value = future.result()
            if value is not None and value != last_multiplier:
                return read_ts, value
        if (multiplier_read is None
                and timestamp - last_multiplier_read >= MULTIPLIER_INTERVAL):
            last_multiplier_read = timestamp
            multiplier_read = (timestamp, telemetry_pool.submit(
                tracker.read_multiplier, frame))
        return None
    ...
    try:
        for timestamp, frame in frames:
            ...
            if state is GameState.PLAYING:
                ...
                change = read_multiplier_change(timestamp, frame)
                if change is not None:
                    read_ts, multiplier = change
                    ...
                    if (last_multiplier is not None
                        ...
                        LOG.say(f"[{read_ts:7.2f}s] !!! COMBO LOST "
                                f"x{last_multiplier} -> x{multiplier}"
                                f"{clock_note}")
                    else:
                        LOG.say(f"[{read_ts:7.2f}s] combo x{multiplier}"
                                f"{clock_note}")
        ...
        if telemetry_pool is not None:
            telemetry_pool.shutdown(wait=False, cancel_futures=True)
